...
import { calcSlamIndex, calcChalkBaselines, combineChalkBaselines } from './scoring.js'
...
function buildCombinedCard(group, allMaps, profs, color) {
  ...
  // Both draws must be on v2 to trust a combined chalk baseline — a mixed v1/v2
  // pairing falls back to the pool-relative index (see calcSlamIndex).
  const siVersion = (d1.slam_index_version ?? 1) === 2 && (d2.slam_index_version ?? 1) === 2 ? 2 : 1
  const chalk = siVersion === 2
    ? combineChalkBaselines(calcChalkBaselines(assembleDrawForUser(d1, [])), calcChalkBaselines(assembleDrawForUser(d2, [])))
    : null
