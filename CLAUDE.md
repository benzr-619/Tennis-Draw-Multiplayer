...
- `.claude/rules/slam-index.md` — Slam Index v2 (2026-08-24) and v3 (2026-08-28): `draws.slam_index_version`, `calcChalkBaselines()` (ELO-favourite Draw Yield baseline + flat-stake odds-favourite Match Yield baseline), why no tuning constant, v1 pool-relative fallback, all call sites — v3 replaces v2's closed-form σ_DY/σ_MY with a persisted Monte Carlo estimate (`src/slam-index-sim.js`, commissioner-triggered, `chalkBaselinesForVersion()` in `scoring.js`)
